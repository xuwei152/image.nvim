...
local adjust_to_aspect_ratio = function(term_size, image_width, image_height, width, height)
  ...
  local percent_orig_width = width == 0 and 0 or pixel_width / image_width
  local percent_orig_height = height == 0 and 0 or pixel_height / image_height

  if width == 0 and height ~= 0 then
    width = math.max(1, math.ceil(pixel_height / term_size.cell_width * aspect_ratio))
    return width, height
  end

  if height == 0 and width ~= 0 then
    height = math.max(1, math.ceil(pixel_width / term_size.cell_height / aspect_ratio))
    return width, height
  end
  ...
  if percent_orig_height > percent_orig_width then
    ...
    local new_width = math.max(width, math.ceil(pixel_height * aspect_ratio / term_size.cell_width))
